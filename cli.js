...
async function printHeader() {
  console.log('')
  try {
    let logo1 = execSync('npx oh-my-logo "MAVERICK" --filled --color --palette-colors "#FF0000,#FF0000"', {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    })
    logo1 = logo1.replace(/\x1b\[0m\x1b\[\?25h\x1b\[K[\s\n]*/g, '\n').trimEnd()
    let logo2 = execSync('npx oh-my-logo "AGENT" --filled --color --palette-colors "#FF0000,#FF0000"', {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'pipe']
    })
    ...
    print('  MAVERICK-AGENT CLI', colors.red + colors.bold)
  }
  await sleep(60)
  console.log('')
}
...
async function startGateway() {
  print('\n🚀 Starting Maverick Agent Gateway...\n', colors.green)
  rl.close()
  ...
  try {
    ...
    agent.cronScheduler.on('execute', async ({ jobId, message, invokeAgent }) => {
      ...
      try {
        ...
        execSync(`osascript -e 'display notification "${escapedMsg}" with title "Maverick" sound name "Glass"'`, { stdio: 'ignore' })
      } catch (e) {
        ...
        for await (const chunk of agent.run({
          ...
          if (chunk.type === 'tool_use') {
            ...
            if (isFirstText) {
              ...
              if (!text) continue
              console.log('')
              console.log(colors.cyan + '  Maverick:' + colors.reset)
              process.stdout.write(padStr)
              curCol = padLen
              isFirstText = false
              lastWasToolUse = false
...
if (args.length === 0) {
  ...
  switch (command) {
    ...
    case '-h':
      await printHeader()
      print('Usage: maverick-agent [command]\n', colors.bold)
      print('Commands:')
    ...
    default:
      ...
      print('Run "maverick-agent help" for usage.')
      rl.close()
      process.exit(1)
  }
}